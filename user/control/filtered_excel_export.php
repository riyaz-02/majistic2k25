...
// Check if user is logged in with Controller, Manage Website, or Super Admin role
...
if (($type === 'all' || $type === 'student') && (!empty($registrations) || $type === 'student')) {
    ...
    if (empty($registrations) && $type === 'student') {
        ...
        // Add summary row for counts
        echo '<tr style="background-color: #c8e6c9; font-weight: bold;">';
        echo '<td colspan="10" style="text-align: right;">TOTALS:</td>';
...
if ($type === 'all' && (!empty($registrations) || !empty($alumni_registrations))) {
    $total_paid = ($student_paid_count ?? 0) + ($alumni_paid_count ?? 0);
    $total_day1 = ($student_day1_checkin_count ?? 0) + ($alumni_day1_checkin_count ?? 0);
    $total_day2 = ($student_day2_checkin_count ?? 0) + ($alumni_day2_checkin_count ?? 0);

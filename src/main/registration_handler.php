...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['payment_check'])) {
    ...
    if ($is_alumni) {
        ...
        $jis_check = $conn->prepare("SELECT * FROM alumni_registrations WHERE jis_id = ?");
        ...
        if ($jis_result->num_rows > 0) {
            ...
            if ($jis_row['payment_status'] == 'Not Paid') {
                ...
                $student_name = $jis_row['alumni_name'];
                $email = $jis_row['email'];
                $mobile = $jis_row['mobile'];
                $department = $jis_row['department'];
                $passout_year = $jis_row['passout_year'];
            ...
            $email_check = $conn->prepare("SELECT email FROM alumni_registrations WHERE email = ?");
            $email_check->bind_param("s", $email);
            $email_check->execute();
            $email_result = $email_check->get_result();
            
            $mobile_check = $conn->prepare("SELECT mobile FROM alumni_registrations WHERE mobile = ?");
            ...
            if ($email_result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO alumni_registrations (alumni_name, gender, jis_id, mobile, email, department, passout_year, current_organization) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssssssss", $student_name, $gender, $jis_id, $mobile, $email, $department, $passout_year, $current_organization);

                if ($stmt->execute()) {
                    $message = "Thank You for your Interest. We'll notify you soon about the ticket payment.";
                    $registration_success = true;
                        ...
                        try {
                            ...
                            $mail->Body = '
                                ...
                                <body>
                                    ...
                                    <p>Thank you for registering for maJIStic 2k25 as an alumni. Please proceed to payment to confirm your participation.</p>
                                    <p><a href="https://skriyaz.com/majistic/src/transaction/payment.php?jis_id=' . $jis_id . '&alumni=1">Click here to complete payment</a></p>
        ...
        // Regular student registration (existing code)
        ...
        if ($jis_result->num_rows > 0) {
            ...
            if ($email_result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO registrations (student_name, gender, jis_id, mobile, email, department, inhouse_competition, competition_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("ssssssss", $student_name, $gender, $jis_id, $mobile, $email, $department, $inhouse_competition, $competition);

                // Update the success message in the registration handler
                if ($stmt->execute()) {
                    $message = "Thank You for your Interest. We'll notify you soon about the ticket payment.";
                    $registration_success = true;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['payment_check'])) {
    ...
    if ($is_alumni) {
        // Check payment status for alumni
        $check_payment_query = $conn->prepare("
            SELECT 
                payment_status, alumni_name, registration_date, payment_id 
            FROM 
                alumni_registrations 
            WHERE 
                jis_id = ?
        ");
        $check_payment_query->bind_param("s", $jis_id);
        ...
        if ($payment_result->num_rows > 0) {
            ...
            if ($row['payment_status'] == 'Not Paid') {
                header("Location: ../../src/transaction/payment.php?jis_id=$jis_id&alumni=1");
                exit();
            } else {
                $message = "Payment already completed for Alumni JIS ID: $jis_id.";
                $email_option = true;
                $payment_id = $row['payment_id'];
            }
        } else {
            $message = "No alumni registration found for the provided JIS ID.";
        }
    } else {
        // Check payment status for regular student
        $check_payment_query = $conn->prepare("
            SELECT 
        ...
        if ($payment_result->num_rows > 0) {
            $row = $payment_result->fetch_assoc();
            if ($row['payment_status'] == 'Not Paid') {
                header("Location: ../../src/transaction/payment.php?jis_id=$jis_id");
                exit();
            } else {
